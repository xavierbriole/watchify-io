    ...
    <nav class="member-nav" role="navigation">
      <div class="nav-wrapper container">
          ...
          if ($user->hasPermission('admin')) {
            echo '<li><a href="admin.php"><i class="material-icons">lock</i></a></li>';
          }
          ?>
    ...
    <div class="section-feed">
      ...
      <div class="row">
              ...
              <ul class="collection with-header followings">
                  <a href="<?php echo HOME_ADDRESS; ?>">
                    <li class="collection-item active">
                      ...
                      <?php if ($feed->getAllBadges($myID) > 0) { ?>
        ...
        <?php if (!$userFollowNobody) { //if user follow someone ?>
		    <?php if (!empty($feed->getFeed(getFollowings($feed, $myID, 'list'), 0, $start, $videosPerPage))) { ?>
	            
              <div id="videos-content">

  	            <?php foreach ($feed->getFeed(getFollowings($feed, $myID, 'list'), 0, $start, $videosPerPage) as $video) { //display each videos ?>
                ...
                <?php
                  ...
                  for ($i = 1; $i <= $pagesCount; $i++) { 
                    if ($i == $currentPage) {
                      ...
                      echo '<a href="https://www.watchify.io/feed/' . $i . '" class="next">' . $i . '</a> ';
                    }
                    else {
                      echo '<a href="https://www.watchify.io/feed/' . $i . '">' . $i . '</a> ';
                    }
                  }
	        ...
	        <?php } else { //if NO VIDEO even if user follow someone ?>
              ...
              <li class="collection-item">
                ...
                <p class="row center no-feed">No Youtuber has released any video yet</p>
              </li>
            </ul>
          <?php }
