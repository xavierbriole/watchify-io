        ...
        <ul id="nav-mobile" class="side-nav">
          ...
          if (!$userFollowNobody) {
            foreach (getFollowings($feed, $myID, 'array') as $key) {
              echo '<li><a href="https://www.watchify.io/channel/' . $feed->getYoutuberInfos($key, 'username') . '">' . $feed->getYoutuberInfos($key, 'name') . '</a></li>';
            }
  ...
  <div class="container">
    <div class="section-feed">
      ...
      <div class="row">
                    ...
                    foreach (getFollowings($feed, $myID, 'array') as $followingID) {
                      ...
                      echo '<a href="https://www.watchify.io/channel/' . $youtuberUsername . '/1">
                              <li class="collection-item ' . (($_SERVER['REQUEST_URI'] === '/channel/' . $youtuberUsername . '/1') ? 'active' : '') . '">
                                ...
                                <span class="new badge red" data-badge-caption="">' . $feed->getBadges($myID, $followingID) . '</span>
                              </li>
                            </a>';
                    }
                  ?>
        ...
        <?php if (!$userFollowNobody) { //if user follow someone ?>
		    <?php if ($feed->videoExist(Input::get('videoID'))) { ?>
	            
	            <?php $youtuberID = $feed->getVideoInfos(Input::get('videoID'), 'youtuberID'); ?>
              ...
              <div id="videos-content">
                ...
                <div class="video-item">
                  ...
                  <ul class="collection">
                      ...
                      <div class="feed-item-infos">
  	                    <div class="feed-item-infos-avatar" style="float: left;">
  	                      <img src="<?php echo $feed->getYoutuberInfos($youtuberID, 'profileIMG'); ?>" class="youtuber-avatar" />
  	                    </div>

  	                    <div class="feed-item-infos-details" style="float:left;">
  	                      <p class="video-title">
  	                        <?php echo $feed->getVideoInfos(Input::get('videoID'), 'title'); ?>
  	                      </p>
  	                      <p class="video-details">
  	                        <a href="https://www.watchify.io/channel/<?php echo $feed->getYoutuberInfos($youtuberID, 'username'); ?>/1" class="author"><?php echo $feed->getYoutuberInfos($youtuberID, 'name'); ?></a>
  	                        • 
  	                        <?php echo number_format($feed->getVideoViews(Input::get('videoID')), 0, '', ' '); ?> views
  	                        • 
  	                        <?php echo getRelativeTime($video->uploaded); ?>
  	                      </p>
  	                    </div>
